...
def predict_time(attributes):
    ...
    x_train, x_test, y_train, y_test = train_test_split(features_DB, labels_DB, test_size=0, random_state=0)
...
def printer_cost(x, y):
    ...
    """
    if x < 12.5 and y < 12.5:
        return small_machine_cost
    else:
        return large_machine_cost


def recycled_savings(dims, consumption, density):
    """
    ...
    if dims[0] > 12.5 or dims[1] > 12.5:
        total = 28 * 28 * dims[2]
        return (total - consumption) * density * powder_cost * 0.5
    else:
        total = 12.5 * 12.5 * dims[2]
        print(total, consumption, density,powder_cost)
        return (total - consumption) * density * powder_cost * 0.5
...
def estimate_cost(shape):
    ...
    cost += recycled_savings(dims, cons, metal_density)
    #print("Recycled: ", "%.0f" % recycled_savings(dims,cons, metal_density), "SEK")
